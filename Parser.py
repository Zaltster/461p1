...
class Lexer:
    def __init__(self, code):
        self.code = code
        self.position = 0
        self.current_char = self.code[self.position] if code else None
        self.tokens = []
...
class Parser:
    ...
    def while_stmt(self):
        ...
        self.expect('COLON')
        body = []
        while self.current_token[0] != 'EOF' and self.peek() != 'ELSE':
            stmt = self.statement()
            if stmt:
                body.append(stmt)
        return AST.WhileStatement(condition, AST.Block(body))
